...
import { incrementGiftCount } from './giftUtils';
...
const Slide: React.FC<SlideProps> = ({ data }) => {
  ...
  // Global state for gift reveals and counts
  const [answeredMap, setAnsweredMap] = useState<Record<number, boolean>>({});
  const [showQRLink, setShowQRLink] = useState<string | null>(null);

  const openGift = (link: string | null) => {
    if (!link) return;
    setShowQRLink(link);
  };

  const markAnswered = (slideId: number) => {
    setAnsweredMap(prev => ({ ...prev, [slideId]: true }));
  };
      ...
      {showQRLink && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="bg-slate-900 p-4 rounded">
            <button onClick={() => setShowQRLink(null)} className="text-slate-300 mb-2">Close</button>
            <div className="p-2 bg-black/90 rounded">
              <img src={`https://api.qrserver.com/v1/create-qr-code/?size=420x420&data=${encodeURIComponent(showQRLink)}`} alt="QR" />
            </div>
          </div>
        </div>
      )}
